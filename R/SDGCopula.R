...
corrfit <- function(orig, syn){
  orig <- tonumeric(orig)
  syn <- tonumeric(syn)
  p1 <- ggcorrplot::ggcorrplot(cor(orig, method=c('spearman')), title = 'Original Dataset', ggtheme = ggplot2::theme(plot.title = element_text(hjust=0.5)))+theme(plot.title = element_text(hjust=0.5))
  p2 <- ggcorrplot::ggcorrplot(cor(syn, method=c('spearman')), title='Synthetic Dataset', ggtheme = ggplot2::theme(plot.title = element_text(hjust=0.5)))
